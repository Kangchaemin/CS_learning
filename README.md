...
* * *
### ★ Context Switching
- 수행 중인 프로세스를 변경할때 CPU의 레지스터 정보가 변경되는 것을 말함
- CPU가 이전 프로세스의 상태를 PCB에 보관하고, 또 다른 프로세스 정보를 PCB에 읽어 레지스터에 적재하는 과정
- **Context** : CPU가 해당 프로세스를 실행하기 위한 프로세스의 정
```
즉, 프로세스가 Ready -> Running 등 상태 변경 시 발생!
```

#### ? 그럼 어떤 정보 덕분에 스위칭이 가능한 것일까?
프로그램 카운터(PC)와 스택 포인터(SP)가 있다.

프로그램 카운터(Program Counter, PC)
> 레지스터 중의 하나로서, **다음에 실행될 명령어의 주소를 가지고 있어 실행할 기계어 코드의 위치를 저장한다.**

스택 포인터(Stack Pointer, SP)
> CPU안에서 **스택에 데이터가 채워진 마지막 위치를 가리키는 레지스터**이다.
